...
extension RequestType {
    ...
    public func build() -> Result<URLRequest> {
        ...
        guard var components = URLComponents(url: url, resolvingAgainstBaseURL: true) else {
            return .failure(ConfluxError.requestError(.invalidURL))
        }
    ...
    public func buildResponse(from data: Data?, response: URLResponse?, error: Error?) -> Result<Response> {
        if let error = error {
            return .failure(ConfluxError.responseError(.connectionError(error)))
        }
        guard let data = data, let urlRequest = response as? HTTPURLResponse else {
            return .failure(ConfluxError.responseError(.noContentProvided))
        }
        guard (200..<300).contains(urlRequest.statusCode) else {
            return .failure(ConfluxError.responseError(.unacceptableStatusCode(urlRequest.statusCode)))
        }
        let response: Response
        do {
            response = try self.response(from: try parse(from: data))
        } catch {
            return .failure(ConfluxError.responseError(.unexpected(error)))
        }
