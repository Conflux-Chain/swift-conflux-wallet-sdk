...
public enum ConfluxChecksumType {
    case eip55
    case wanchain
}

public struct ConfluxChecksum {
    public static func computeString(for data: Data, type: ConfluxChecksumType) -> String {
        let addressString = data.hexString
        let hashInput = addressString.data(using: .ascii)!
        let hash = hashInput.sha3(.keccak256).hexString
